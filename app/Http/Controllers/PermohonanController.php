...
class PermohonanController extends Controller
{
    public function index()
    {
        ...
        elseif ($user->role === 'kepala_admin') {
            $permohonans = Permohonan::orderBy('created_at', 'desc')->get();
            return view('dashboard.admin', compact('permohonans'));
        }
    ...
    public function create() { return view('permohonan.create'); }

    public function store(Request $request)
    {
        $request->validate([
            'nama_pic' => 'required', 'kontak_pic' => 'required', 'kendaraan_dibutuhkan' => 'required',
            'titik_jemput' => 'required', 'tujuan' => 'required', 'waktu_berangkat' => 'required',
            'waktu_kembali' => 'required', 'jumlah_penumpang' => 'required', 'file_surat' => 'required|mimes:pdf,jpg,jpeg,png|max:2048',
        ...
        $permohonan = Permohonan::create([
            'user_id' => Auth::id(), 'nama_pic' => $request->nama_pic, 'kontak_pic' => $request->kontak_pic,
            'kendaraan_dibutuhkan' => $request->kendaraan_dibutuhkan, 'titik_jemput' => $request->titik_jemput,
            'tujuan' => $request->tujuan, 'waktu_berangkat' => $request->waktu_berangkat,
            'waktu_kembali' => $request->waktu_kembali, 'jumlah_penumpang' => $request->jumlah_penumpang,
            'file_surat_penugasan' => $filePath, 'anggaran_diajukan' => $request->anggaran_diajukan,
            'status_permohonan' => 'Menunggu Validasi Admin',
        ]);

        $admins = User::where('role', 'kepala_admin')->get();
        foreach ($admins as $admin) { $admin->notify(new StatusPermohonanNotification($permohonan, 'Permohonan baru dari ' . $request->nama_pic)); }
...
$user->notify(new StatusPermohonanNotification($permohonan, 'Pengajuan berhasil dikirim.'));
    ...
    public function validasiAdminForm($id) { return view('permohonan.validasi_admin', ['permohonan' => Permohonan::findOrFail($id)]); }
    ...
    public function validasiAdminProses(Request $request, $id)
    {
        ...
        if ($request->status_permohonan === 'Menunggu Proses SPSI') {
            ...
            foreach ($spsis as $spsi) { $spsi->notify(new StatusPermohonanNotification($permohonan, 'Butuh alokasi armada.')); }
    ...
    public function prosesKeuanganForm($id) { return view('permohonan.proses_keuangan', ['permohonan' => Permohonan::with(['kendaraan', 'pengemudi'])->findOrFail($id)]); }
    ...
    public function finalisasiAdminForm($id) { return view('permohonan.finalisasi_admin', ['permohonan' => Permohonan::with(['kendaraan', 'pengemudi', 'user'])->findOrFail($id)]); }
    ...
    public function show($id) { return view('permohonan.show', ['permohonan' => Permohonan::with(['kendaraan', 'pengemudi'])->where('user_id', Auth::id())->findOrFail($id)]); }
