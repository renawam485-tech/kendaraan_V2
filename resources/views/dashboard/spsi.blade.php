<x-app-layout>
                    ...
                    @forelse($permohonans as $i => $p)
                        @php
                            $sc = match($p->status_permohonan) {
                                'Menunggu Proses SPSI' => 'bg-amber-50 text-amber-700 border-amber-200',
                        ...
                        <div class="p-4 hover:bg-slate-50 transition searchable-row"
                                ...
                                <div class="flex items-start gap-2">
                                    ...
                                    <div>
                                        ...
                                        @if($p->kategori_kegiatan)
                                            <span class="text-[10px] font-bold text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded mt-1 inline-block">{{ $p->kategori_kegiatan }}</span>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="mt-3 pl-8">
                                @if($p->status_permohonan === 'Menunggu Proses SPSI')
                                    <a href="{{ route('permohonan.proses_spsi', $p->id) }}" class="w-full flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg text-xs transition">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-slate-50 border-b border-gray-200">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Instruksi Admin</th>
                            ...
                            @forelse($permohonans as $i => $p)
                                ...
                                <tr class="hover:bg-blue-50/20 transition-colors searchable-row"
                                    ...
                                    <td class="px-4 py-3.5 whitespace-nowrap">
                                        ...
                                        <p class="text-xs text-gray-400">s/d {{ \Carbon\Carbon::parse($p->waktu_kembali)->format('d M Y') }}</p>
                                    </td>
                                    <td class="px-4 py-3.5">
                                        @if($p->kategori_kegiatan)
    ...
    document.getElementById('searchInput').addEventListener('input', function() {
        ...
        let vis = 0; const total = {{ $permohonans->count() }};
