...
<body>
    ...
    <div class="header">
        ...
        <h2>{{ $judul }}</h2>
        <p>Dicetak pada: {{ now()->translatedFormat('l, d F Y H:i') }} WIB &nbsp;|&nbsp; Oleh: {{ $user->name }}
    ...
    <table>
        ...
        <tbody>
            @forelse($data as $i => $p)
                @php
                    $badgeClass = match ($p->status_permohonan) {
                        ...
                        'Ditolak' => 'badge-red',
                        'Menunggu Pengembalian Dana', 'Menunggu Verifikasi Pengembalian' => 'badge-orange',
                ...
                @if ($user->role === 'keuangan')
                    ...
                    <tr>
                        ...
                        <td><span class="badge {{ $badgeClass }}">{{ $p->status_permohonan }}</span></td>
                    </tr>
                @elseif($user->role === 'spsi')
                    <tr>
                        <td>{{ $i + 1 }}</td>
                        <td>{{ $p->nama_pic }}</td>
                        <td>{{ $p->tujuan }}</td>
                        <td>{{ $p->kendaraan?->nama_kendaraan ?? ($p->kendaraan_vendor ?? '-') }}</td>
            ...
            @empty
                <tr>
                    <td colspan="10" style="text-align:center; padding:20px; color:#9ca3af;">Tidak ada data.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
