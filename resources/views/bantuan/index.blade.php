    ...
    <div class="py-8 bg-gray-50 min-h-screen">
        <div class="w-full px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @elseif($role === 'spsi')
                {{-- RINGKASAN TUGAS --}}
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    @foreach ([['Alokasi Armada', 'Tugaskan kendaraan dan pengemudi untuk permohonan yang sudah divalidasi Admin.', 'bi-signpost-split', 'blue'], ['Monitoring Armada', 'Pantau seluruh armada yang sedang berjalan hingga perjalanan dinyatakan selesai.', 'bi-binoculars', 'slate'], ['Master Data', 'Kelola data kendaraan (mobil), pengemudi (supir), dan armada (vendor/kampus).', 'bi-database', 'emerald']] as [$judul, $desc, $icon, $warna])
                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                            <div
                                class="w-10 h-10 rounded-lg {{ $warna === 'blue' ? 'bg-blue-50 text-blue-600' : ($warna === 'emerald' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600') }} flex items-center justify-center mb-3">
                                <i class="bi {{ $icon }} text-lg"></i>
                            </div>
                            <h4 class="font-bold text-gray-800 text-sm mb-1">{{ $judul }}</h4>
                            <p class="text-xs text-gray-500 leading-relaxed">{{ $desc }}</p>
                ...
                {{-- PANDUAN MASTER DATA SPSI (BARU) --}}
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-emerald-50 flex items-center gap-2">
                        <i class="bi bi-database-gear text-emerald-600 text-lg"></i>
                        <h3 class="font-bold text-gray-800">Panduan: Master Data (Mobil, Supir, Kendaraan)</h3>
                    </div>
                    <div class="p-6 space-y-5">
                        <p class="text-sm text-gray-500">Menu <strong>Master Data</strong> digunakan untuk mengelola data aset dan sumber daya armada. Berikut penjelasan detail setiap submenu:</p>
                        
                        {{-- MOBIL (Kendaraan Kampus) --}}
                        <div class="border-l-4 border-blue-400 pl-4">
                            <h4 class="font-bold text-gray-800 text-sm flex items-center gap-2 mb-2"><i class="bi bi-car-front text-blue-600"></i> 1. Data Mobil (Kendaraan Aset Kampus)</h4>
                            <ul class="space-y-2 text-sm text-gray-600 ml-4 list-disc">
                                <li><strong>Tambah Mobil Baru:</strong> Klik tombol "Tambah Mobil". Isi <strong>Nama Kendaraan</strong> (contoh: Toyota Avanza), <strong>Plat Nomor</strong> (unik, contoh: B 1234 ABC), <strong>Kapasitas Penumpang</strong> (jumlah orang), <strong>Fasilitas</strong> (AC, TV, dll - opsional), dan <strong>Status</strong> (Tersedia/Dipinjam/Maintenance).</li>
                                <li><strong>Edit/Update:</strong> Klik ikon pensil pada baris mobil yang ingin diubah. Hanya mobil dengan status "Tersedia" yang bisa dialokasikan ke permohonan.</li>
                                <li><strong>Hapus Mobil:</strong> Klik ikon tempat sampah. Mobil <strong>tidak bisa dihapus</strong> jika sedang memiliki riwayat peminjaman aktif (status Dipinjam) atau sudah pernah digunakan dalam permohonan yang sudah berstatus Selesai. Sistem akan menampilkan peringatan.</li>
                                <li><strong>Status Maintenance:</strong> Jika mobil sedang dalam perbaikan, ubah status menjadi "Maintenance". Mobil dengan status Maintenance <strong>tidak akan muncul</strong> di dropdown alokasi armada.</li>
                            </ul>
                        </div>

                        {{-- SUPIR (Pengemudi) --}}
                        <div class="border-l-4 border-amber-400 pl-4">
                            <h4 class="font-bold text-gray-800 text-sm flex items-center gap-2 mb-2"><i class="bi bi-person-badge text-amber-600"></i> 2. Data Supir (Pengemudi)</h4>
                            <ul class="space-y-2 text-sm text-gray-600 ml-4 list-disc">
                                <li><strong>Tambah Supir Baru:</strong> Klik "Tambah Supir". Isi <strong>Nama Lengkap</strong>, <strong>Nomor Telepon</strong> (format +62...), <strong>Nomor SIM</strong> (wajib), <strong>Alamat</strong>, dan <strong>Status</strong> (Tersedia/Bertugas/Cuti).</li>
                                <li><strong>Edit/Update:</strong> Ubah data supir jika ada perubahan kontak atau status. Status "Bertugas" diberikan otomatis oleh sistem saat supir dialokasikan ke permohonan dan akan kembali menjadi "Tersedia" setelah perjalanan selesai.</li>
                                <li><strong>Hapus Supir:</strong> Supir hanya bisa dihapus jika <strong>tidak memiliki riwayat tugas</strong> (belum pernah dialokasikan) atau sudah tidak aktif.</li>
                                <li><strong>Validasi SIM:</strong> Pastikan SIM masih berlaku sebelum menugaskan supir. Sistem tidak melakukan validasi otomatis, jadi perlu dicek manual.</li>
                            </ul>
                        </div>

                        {{-- KENDARAAN (Armada Vendor + Kampus Terintegrasi) --}}
                        <div class="border-l-4 border-purple-400 pl-4">
                            <h4 class="font-bold text-gray-800 text-sm flex items-center gap-2 mb-2"><i class="bi bi-truck-front text-purple-600"></i> 3. Data Kendaraan (Armada Vendor & Integrasi)</h4>
                            <ul class="space-y-2 text-sm text-gray-600 ml-4 list-disc">
                                <li><strong>Kendaraan Vendor:</strong> Untuk armada dari pihak ketiga (sewa), tambahkan dengan memilih <strong>Sumber = Vendor</strong>. Isi <strong>Nama Vendor</strong> (PT. XYZ Rent), <strong>Nama Kendaraan</strong>, <strong>Plat Nomor</strong>, <strong>Kapasitas</strong>, dan <strong>Biaya Sewa/Hari</strong> (opsional, untuk referensi).</li>
                                <li><strong>Integrasi dengan Data Mobil:</strong> Kendaraan aset kampus yang sudah ada di menu "Data Mobil" secara otomatis muncul di sini dengan Sumber = Kampus. Anda tidak perlu menambahkan ulang.</li>
                                <li><strong>Fitur Pencarian:</strong> Gunakan filter berdasarkan <strong>Sumber</strong> (Kampus/Vendor) dan <strong>Status</strong> untuk mencari kendaraan yang sesuai dengan kebutuhan pemohon.</li>
                                <li><strong>Ketersediaan Armada:</strong> Hanya kendaraan dengan status "Tersedia" yang muncul saat alokasi. Jika semua kendaraan kampus sedang dipinjam, gunakan opsi Vendor.</li>
                            </ul>
                        </div>

                        <div class="bg-amber-50 rounded-lg p-3 text-xs text-amber-700 mt-3">
                            <i class="bi bi-info-circle-fill mr-1"></i> 
                            <strong>Catatan Penting:</strong> Perubahan status kendaraan dan supir (Tersedia → Dipinjam/Bertugas) dilakukan <strong>otomatis oleh sistem</strong> saat SPSI mengalokasikan armada. Jangan mengubah status secara manual jika armada sedang digunakan, karena akan mengganggu pelacakan.
                        </div>
                    </div>
                </div>
                ...
                {{-- FAQ SPSI (ditambah FAQ Master Data) --}}
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-slate-50 flex items-center gap-2">
                        <i class="bi bi-question-circle text-blue-600"></i>
                        <h3 class="font-bold text-gray-800">Pertanyaan Umum (FAQ)</h3>
                    ...
                    <div class="divide-y divide-gray-100">
                        @foreach ([
                            ['Tidak ada kendaraan kampus yang tersedia, apa yang dilakukan?', 'Pilih sumber armada <strong>Vendor</strong> dan pilih kendaraan dari daftar mitra vendor yang tersedia. Jika tidak ada kendaraan vendor pula, hubungi Kepala Admin untuk tindak lanjut.'],
                            ['Apakah pengemudi wajib diisi?', 'Tidak wajib. Jika pemohon mengendarai sendiri atau menggunakan sistem lepas kunci, biarkan kolom pengemudi kosong (pilih opsi "— Tanpa Pengemudi —").'],
                            ['Bagaimana jika ada instruksi khusus dari Admin?', 'Instruksi Admin akan tampil dalam kotak kuning di halaman alokasi. Baca dan ikuti instruksi tersebut sebelum menentukan pilihan kendaraan dan pengemudi.'],
                            ['Apakah estimasi biaya harus diisi untuk kegiatan Non SITH?', 'Tidak. Untuk kegiatan Non SITH, kolom estimasi biaya tersembunyi dan nilainya otomatis 0 (nol) karena biaya ditanggung pemohon sendiri.'],
                            ['Bagaimana cara menambah mobil vendor baru?', 'Masuk ke menu <strong>Master Data → Kendaraan</strong>, klik "Tambah", pilih Sumber = Vendor, lalu isi data kendaraan dan nama vendor.'],
                            ['Supir tidak bisa dihapus padahal sudah tidak aktif?', 'Supir yang pernah memiliki riwayat tugas (pernah dialokasikan) tidak dapat dihapus untuk menjaga integritas data audit. Ubah statusnya menjadi "Cuti" atau nonaktifkan secara manual melalui edit data.'],
                            ['Apakah data mobil yang sudah dihapus bisa dikembalikan?', 'Tidak. Penghapusan bersifat permanen. Sebaiknya ubah status menjadi "Maintenance" daripada menghapus jika masih mungkin digunakan di masa depan.'],
                        ] as [$q, $a])
                            <div x-data="{ open: false }" class="px-6 py-4">
                                <button @click="open = !open"
                                    class="w-full flex items-center justify-between gap-3 text-left">
            ...
            {{-- =============================================
                 SUPER ADMIN (GABUNGAN SEMUA BANTUAN)
            ============================================= --}}
            @elseif($role === 'super_admin')
                {{-- RINGKASAN AKSES SUPER ADMIN --}}
                <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200 p-6">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <h3 class="font-black text-gray-800 text-lg">Akses Super Administrator</h3>
                    </div>
                    <p class="text-sm text-gray-700">Anda memiliki akses penuh ke seluruh fitur sistem. Berikut adalah rangkuman panduan dari semua peran (Kepala Admin, SPSI, Keuangan) yang dapat Anda kelola dan pantau.</p>
                </div>

                {{-- TAB NAVIGASI UNTUK SUPER ADMIN (Menggunakan Alpine JS) --}}
                <div x-data="{ tab: 'admin' }" class="space-y-6">
                    <div class="flex flex-wrap gap-2 border-b border-gray-200 pb-2">
                        <button @click="tab = 'admin'" :class="tab === 'admin' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                            class="px-5 py-2 rounded-t-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-sm">
                            <i class="bi bi-shield-check"></i> Admin
                        </button>
                        <button @click="tab = 'spsi'" :class="tab === 'spsi' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                            class="px-5 py-2 rounded-t-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-sm">
                            <i class="bi bi-truck-front"></i> SPSI
                        </button>
                        <button @click="tab = 'keuangan'" :class="tab === 'keuangan' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                            class="px-5 py-2 rounded-t-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-sm">
                            <i class="bi bi-cash-coin"></i> Keuangan
                        </button>
                        <button @click="tab = 'pengguna'" :class="tab === 'pengguna' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                            class="px-5 py-2 rounded-t-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-sm">
                    ...
                    <div x-show="tab === 'admin'" x-cloak>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            @foreach ([['Validasi Permohonan', 'Tinjau dan kategorikan setiap pengajuan masuk.', 'bi-clipboard2-check', 'blue'], ['Finalisasi Penerbitan', 'Konfirmasi akhir dan terbitkan surat izin.', 'bi-send-check', 'purple'], ['Pantau Arsip', 'Lihat seluruh riwayat permohonan.', 'bi-archive', 'slate']] as [$judul, $desc, $icon, $warna])
                                @php $map = ['blue' => 'bg-blue-50 text-blue-600', 'purple' => 'bg-purple-50 text-purple-600', 'slate' => 'bg-slate-100 text-slate-600']; @endphp
                                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                            <div class="px-6 py-4 bg-blue-50 border-b border-gray-100 flex items-center gap-2">
                                <i class="bi bi-shield-check text-blue-600"></i>
                                <h3 class="font-bold text-gray-800">Panduan Validasi & Finalisasi (Admin)</h3>
                            </div>
                            <div class="p-5 space-y-3 text-sm text-gray-600">
                                <p><strong>Validasi:</strong> Pilih kategori Dinas SITH atau Non SITH, isi instruksi jika perlu → Setujui & Teruskan ke SPSI.</p>
                                <p><strong>Finalisasi:</strong> Periksa rangkuman akhir (kendaraan, pengemudi, RAB) → Setujui & Terbitkan Surat ke Pengguna.</p>
                                <p><strong>Arsip:</strong> Menu ini menampilkan semua permohonan dengan status Disetujui, Ditolak, Selesai, dan yang berkaitan dengan pengembalian dana.</p>
                            </div>
                        </div>
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            <div class="px-6 py-4 bg-slate-50 flex items-center gap-2">
                                <i class="bi bi-question-circle text-blue-600"></i>
                                <h3 class="font-bold text-gray-800">FAQ Admin</h3>
                            </div>
                            <div class="divide-y divide-gray-100">
                                @foreach ([['Apa bedanya Dinas SITH dan Non SITH?', 'Dinas SITH → biaya instansi, proses Keuangan. Non SITH → biaya pribadi, langsung ke SPSI lalu finalisasi.'], ['Bagaimana jika permohonan yang sudah divalidasi perlu diubah?', 'Hubungi SPSI langsung jika armada belum dialokasikan.'], ['Di mana melihat riwayat?', 'Menu Arsip & Riwayat di sidebar.']] as [$q, $a])
                                    <div x-data="{ open: false }" class="px-6 py-3">
                                        <button @click="open = !open" class="w-full flex justify-between text-left"><span class="font-semibold text-sm">{{ $q }}</span><i :class="open ? 'bi-dash-circle' : 'bi-plus-circle'" class="text-gray-400"></i></button>
                                        <div x-show="open" x-collapse class="mt-2 text-sm text-gray-600">{!! $a !!}</div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    {{-- TAB SPSI (dengan Master Data) --}}
                    <div x-show="tab === 'spsi'" x-cloak>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            @foreach ([['Alokasi Armada', 'Tugaskan kendaraan dan pengemudi.', 'bi-signpost-split', 'blue'], ['Monitoring Armada', 'Pantau armada yang sedang berjalan.', 'bi-binoculars', 'slate'], ['Master Data', 'Kelola mobil, supir, dan armada vendor.', 'bi-database', 'emerald']] as [$judul, $desc, $icon, $warna])
                                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                                    <div class="w-10 h-10 rounded-lg {{ $warna === 'blue' ? 'bg-blue-50 text-blue-600' : ($warna === 'emerald' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600') }} flex items-center justify-center mb-3">
                                        <i class="bi {{ $icon }} text-lg"></i>
                                    </div>
                                    <h4 class="font-bold text-gray-800 text-sm mb-1">{{ $judul }}</h4>
                                    <p class="text-xs text-gray-500">{{ $desc }}</p>
                        ...
                        {{-- PANDUAN MASTER DATA UNTUK SUPER ADMIN (sama seperti SPSI) --}}
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                            <div class="px-6 py-4 bg-emerald-50 border-b border-gray-100 flex items-center gap-2">
                                <i class="bi bi-database-gear text-emerald-600"></i>
                                <h3 class="font-bold text-gray-800">Master Data: Mobil, Supir, Kendaraan Vendor</h3>
                            </div>
                            <div class="p-5 space-y-4 text-sm text-gray-600">
                                <div><strong>Mobil (Kampus):</strong> Tambah/edit/hapus mobil aset internal. Status: Tersedia, Dipinjam, Maintenance. Hanya yang Tersedia muncul di alokasi.</div>
                                <div><strong>Supir:</strong> Kelola data pengemudi (nama, kontak, SIM). Status berubah otomatis saat ditugaskan.</div>
                                <div><strong>Kendaraan Vendor:</strong> Untuk armada sewa, pilih Sumber = Vendor. Isi nama vendor dan biaya sewa (referensi).</div>
                                <div class="bg-amber-50 p-2 rounded"><i class="bi bi-info-circle"></i> Status kendaraan dan supir berubah otomatis saat alokasi dan setelah perjalanan selesai.</div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            <div class="px-6 py-4 bg-slate-50 flex items-center gap-2">
                                <i class="bi bi-question-circle text-blue-600"></i>
                                <h3 class="font-bold text-gray-800">FAQ SPSI</h3>
                            </div>
                            <div class="divide-y divide-gray-100">
                                @foreach ([['Tidak ada kendaraan kampus tersedia?', 'Gunakan sumber Vendor. Jika tidak ada, hubungi Admin.'], ['Apakah pengemudi wajib diisi?', 'Tidak. Kosongkan jika pemohon menyetir sendiri.'], ['Bagaimana cara menambah mobil vendor?', 'Master Data → Kendaraan → Tambah, pilih Sumber = Vendor.']] as [$q, $a])
                                    <div x-data="{ open: false }" class="px-6 py-3">
                                        <button @click="open = !open" class="w-full flex justify-between text-left"><span class="font-semibold text-sm">{{ $q }}</span><i :class="open ? 'bi-dash-circle' : 'bi-plus-circle'" class="text-gray-400"></i></button>
                                        <div x-show="open" x-collapse class="mt-2 text-sm text-gray-600">{{ $a }}</div>
                                    </div>
                    ...
                    <div x-show="tab === 'keuangan'" x-cloak>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            @foreach ([['Persetujuan RAB', 'Tetapkan RAB dan mekanisme pembayaran.', 'bi-receipt-cutoff', 'blue'], ['Verifikasi Pengembalian', 'Cek bukti pengembalian sisa dana.', 'bi-cash-stack', 'emerald']] as [$judul, $desc, $icon, $warna])
                                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            <div class="px-6 py-4 bg-slate-50 flex items-center gap-2">
                                <i class="bi bi-question-circle text-blue-600"></i>
                                <h3 class="font-bold text-gray-800">FAQ Keuangan</h3>
                            </div>
                            <div class="divide-y divide-gray-100">
                                @foreach ([['Apakah Non SITH masuk ke Keuangan?', 'Tidak. Langsung ke finalisasi Admin dengan RAB 0.'], ['Bolehkah mengubah nilai RAB dari estimasi SPSI?', 'Ya, sesuai kebijakan keuangan.'], ['Apa beda Cash, Cashless, Reimburse?', 'Cash/Cashless ada potensi pengembalian sisa dana. Reimburse langsung selesai tanpa pengembalian.']] as [$q, $a])
                                    <div x-data="{ open: false }" class="px-6 py-3">
                                        <button @click="open = !open" class="w-full flex justify-between text-left"><span class="font-semibold text-sm">{{ $q }}</span><i :class="open ? 'bi-dash-circle' : 'bi-plus-circle'" class="text-gray-400"></i></button>
                                        <div x-show="open" x-collapse class="mt-2 text-sm text-gray-600">{{ $a }}</div>
                                    </div>
                    ...
                    {{-- TAB PENGGUNA --}}
                    <div x-show="tab === 'pengguna'" x-cloak>
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            <div class="px-6 py-4 bg-blue-50 flex items-center gap-2">
                                <i class="bi bi-map text-blue-600"></i>
                                <h3 class="font-bold text-gray-800">Alur Pengajuan untuk Pengguna</h3>
                            </div>
                            <div class="p-5 text-sm text-gray-600 space-y-2">
                                <p>1. Buat Pengajuan → 2. Validasi Admin → 3. Alokasi SPSI → 4. (Dinas SITH) Proses Keuangan → 5. Finalisasi Admin → 6. Disetujui → 7. Cetak Surat → 8. Laksanakan Perjalanan → 9. Selesaikan & LPJ → 10. Pengembalian Dana (jika ada).</p>
                                <p class="text-xs text-gray-400 mt-3">Untuk panduan lebih detail, silakan login sebagai pengguna atau hubungi tim IT.</p>
                            </div>
                        </div>
                    </div>
                </div>
