...
<body class="font-sans antialiased">
    ...
    <div x-data="{ 
         ...
         <div x-show="open" style="display: none;" class="fixed inset-0 bg-black bg-opacity-40 z-40 transition-opacity" @click="open = false"></div>
         ...
         <div class="fixed top-0 right-0 h-full w-full sm:w-96 bg-white shadow-2xl z-50 transform transition-transform duration-300 flex flex-col"
              ...
              <div x-show="view === 'list'" class="p-4 border-b flex justify-between items-center bg-gray-50">
                  ...
                  <button @click="open = false" class="text-gray-400 hover:text-red-500 text-3xl font-light focus:outline-none">&times;</button>
              </div>

              <div x-show="view === 'detail'" style="display: none;" class="p-4 border-b flex items-center bg-white shadow-sm z-10">
                  <button @click="view = 'list'" class="text-blue-600 font-bold text-sm flex items-center hover:text-blue-800 transition focus:outline-none py-1">
                      <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> Kembali
                  </button>
              </div>
              ...
              <div x-show="view === 'list'" class="flex-1 overflow-y-auto flex flex-col bg-white">
                  <div class="flex border-b text-xs flex-shrink-0 bg-white sticky top-0 z-10 shadow-sm">
                      <button @click="bacaSemua()" class="flex-1 py-4 px-2 text-blue-600 hover:bg-blue-50 font-bold border-r transition focus:outline-none">✓ Baca Semua</button>
                      <button @click="hapusTerbaca()" class="flex-1 py-4 px-2 text-red-600 hover:bg-red-50 font-bold transition focus:outline-none">🗑 Hapus Terbaca</button>
                  </div>
                  @forelse(auth()->user()->notifications as $notif)
                     ...
                     <a href="#" 
                        ...
                        class="block p-4 border-b hover:bg-gray-100 transition {{ $notif->read_at ? 'opacity-60 bg-white read-item' : 'bg-purple-50 unread-item' }}">
                         ...
                         <div class="font-bold text-sm {{ $notif->read_at ? 'text-gray-600' : 'text-purple-700' }}">{{ $notif->data['status'] ?? 'Info Sistem' }}</div>
                         ...
                         <div class="text-[11px] text-gray-500 mt-2 flex justify-between items-center">
                             ...
                             @if(!$notif->read_at) <span class="w-2.5 h-2.5 bg-red-500 rounded-full unread-dot shadow-sm"></span> @endif
                         </div>
                     </a>
                  @empty
              ...
              <div x-show="view === 'detail'" style="display: none;" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="flex-1 bg-gray-50 overflow-y-auto p-4 sm:p-6">
                  ...
                  <p class="text-xs text-gray-400 mt-2 mb-6 flex items-center">
                      <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> <span x-text="activeTime"></span>
                  </p>
                  
                  <div class="bg-white border border-gray-200 p-4 sm:p-5 rounded-lg shadow-sm text-gray-700 text-sm leading-relaxed mb-8" x-text="activeMessage"></div>

                  <a :href="activeUrl" class="block w-full text-center bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 sm:py-4 px-4 rounded-lg shadow-md transition focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2">
    ...
    <script type="module">
        ...
        window.showCompactToast = function(title, message) {
            ...
            toast.className = 'bg-white border border-gray-200 shadow-2xl rounded-lg p-3 sm:p-4 w-[90vw] sm:w-80 max-w-sm transform transition-all duration-500 translate-x-[120%] flex items-start gap-3 relative';
            toast.innerHTML = `
                    ...
                    .notification((notification) => {
                        ...
                        if (autoUpdatePaths.includes(window.location.pathname)) {
                            ...
                            fetch(window.location.href)
                                ...
                                .catch(error => console.error('Gagal memperbarui halaman secara real-time:', error));
                        }
                    });
            }
        });
