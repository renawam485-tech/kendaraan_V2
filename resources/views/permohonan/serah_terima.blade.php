    ...
    <div class="py-6 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                @foreach([
                    [$pending->total(),            'Perlu Serah Terima',  'bi-key-fill',          'bg-amber-50',  'text-amber-600'],
                    [$menungguMulai->total(),       'Menunggu User Mulai', 'bi-person-walking',    'bg-yellow-50', 'text-yellow-600'],
                    [$berlangsung->total(),         'Dalam Perjalanan',    'bi-geo-alt-fill',      'bg-teal-50',   'text-teal-600'],
                    [$menungguKonfirmasi->total(),  'Menunggu Konfirmasi', 'bi-arrow-return-left', 'bg-indigo-50', 'text-indigo-600'],
                    [$riwayat->total(),             'Riwayat',             'bi-archive',           'bg-slate-100', 'text-slate-600'],
            ...
            @php
                ...
                $tabAlerts = [
                    'pending'     => ['bg-amber-50 border-amber-100',   'text-amber-500',  'bi-info-circle-fill',    'text-amber-700',   'Permohonan sudah <strong>disetujui Admin</strong>. Serahkan kunci fisik kepada pemohon dan klik tombol konfirmasi.'],
                    'menunggu'    => ['bg-yellow-50 border-yellow-100', 'text-yellow-500', 'bi-info-circle-fill',    'text-yellow-700',  'Kunci sudah diserahkan. Menunggu pemohon klik <strong>"Mulai Perjalanan"</strong>.'],
                    'berlangsung' => ['bg-teal-50 border-teal-100',     'text-teal-500',   'bi-geo-alt-fill',        'text-teal-700',    'Kendaraan sedang dalam perjalanan aktif.'],
                    'konfirmasi'  => ['bg-indigo-50 border-indigo-100', 'text-indigo-500', 'bi-arrow-return-left',   'text-indigo-700',  'Pemohon melaporkan sudah kembali. Periksa kondisi kendaraan lalu konfirmasi.'],
                    'riwayat'     => ['bg-slate-50 border-slate-200',   'text-slate-500',  'bi-archive',             'text-gray-600',    'Riwayat perjalanan yang sudah selesai atau ditutup.'],
                ];

                $tabTableConfig = [
                    'pending'     => ['items' => $pending,            'type' => 'pending',     'showActions' => true,  'actionRoute' => 'permohonan.serah_terima_kunci',  'actionText' => 'Serah Terima', 'actionColor' => 'bg-amber-500 hover:bg-amber-600'],
                    ...
                    'konfirmasi'  => ['items' => $menungguKonfirmasi, 'type' => 'konfirmasi',  'showActions' => true,  'actionRoute' => 'permohonan.konfirmasi_kembali',  'actionText' => 'Konfirmasi',  'actionColor' => 'bg-indigo-600 hover:bg-indigo-700'],
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                {{-- Table via partial --}}
                @include('permohonan.partials.tabel-serah-terima', array_merge(
                    $tabTableConfig[$activeTab],
                    [
