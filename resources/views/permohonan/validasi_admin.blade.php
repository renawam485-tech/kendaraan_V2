            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6">
                    <form action="{{ route('permohonan.validasi_admin_proses', $permohonan->id) }}" method="POST"
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Kategori Kegiatan <span
                                    class="text-red-500">*</span></label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach ([['Dinas SITH', 'Ditanggung Instansi', 'bi-building-check', 'text-green-600', 'bg-green-50 border-green-300'], ['Non SITH', 'Biaya Pribadi Pemohon', 'bi-person-check', 'text-gray-600', 'bg-gray-50 border-gray-300']] as [$val, $desc, $icon, $tc, $bc])
                                    <label
                                        class="flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition hover:shadow-md {{ $bc }} has-[:checked]:ring-2 has-[:checked]:ring-blue-400">
                                        <input type="radio" name="kategori_kegiatan" value="{{ $val }}"
                                            {{ old('kategori_kegiatan') === $val ? 'checked' : '' }}
                                            class="mt-0.5 text-blue-600 focus:ring-blue-500">
                                        <div>
                                            <p class="font-bold text-gray-800 text-sm flex items-center gap-1.5"><i
                                                    class="bi {{ $icon }} {{ $tc }}"></i>
                                                {{ $val }}</p>
                            ...
                            <p class="text-xs text-gray-500 mt-2"><i class="bi bi-info-circle mr-1"></i>Memilih
                                <strong>Non SITH</strong> akan otomatis mem-bypass proses Keuangan dan menol-kan
                                anggaran.</p>
                        </div>

                        <div>
                            <label for="rekomendasi_admin"
                                class="block text-sm font-bold text-gray-800 mb-1.5">Instruksi untuk SPSI <span
