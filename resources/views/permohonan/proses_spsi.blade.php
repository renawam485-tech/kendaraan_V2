<x-app-layout>
    ...
    <style>
        ...
        .select2-container--default .select2-selection--single:focus { border-color: #3b82f6; box-shadow: 0 0 0 2px rgba(59,130,246,.2); }
        ...
        .select2-dropdown { border: 1px solid #3b82f6; border-radius: .5rem; box-shadow: 0 10px 25px rgba(0,0,0,.1); }
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-5">
                ...
                <div class="p-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <div class="{{ $permohonan->kategori_kegiatan === 'Non SITH' ? 'bg-gray-50 border-gray-200' : 'bg-green-50 border-green-200' }} rounded-xl border p-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">Jenis Kegiatan</p>
                        <p class="font-black text-gray-800 text-sm">{{ $permohonan->kategori_kegiatan === 'Non SITH' ? 'NON-DINAS' : 'DINAS SITH' }}</p>
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
                        ...
                        <p class="font-semibold text-gray-800 text-sm">{{ \Carbon\Carbon::parse($permohonan->waktu_berangkat)->format('d M, H:i') }}</p>
                        <p class="text-xs text-gray-500">s/d {{ \Carbon\Carbon::parse($permohonan->waktu_kembali)->format('d M, H:i') }}</p>
                    </div>
                </div>
                @if($permohonan->rekomendasi_admin)
                    <div class="px-5 pb-5">
                        <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
                            ...
                            <p class="text-sm text-amber-800"><strong>Instruksi Admin:</strong> {{ $permohonan->rekomendasi_admin }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('permohonan.proses_spsi_submit', $permohonan->id) }}" method="POST" class="space-y-6">
                        @csrf @method('PUT')
                        ...
                        {{-- PILIH SUMBER --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Sumber Armada <span class="text-red-500">*</span></label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach([
                                    ['Kampus','Mobil Kampus (Aset Internal)','bi-building','text-blue-600','bg-blue-50 border-blue-200'],
                                    ['Vendor','Sewa Vendor Luar','bi-shop','text-orange-600','bg-orange-50 border-orange-200'],
                                ] as [$val,$lbl,$icon,$tc,$bc])
                                    <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition hover:shadow-md {{ $bc }} has-[:checked]:ring-2 has-[:checked]:ring-blue-400">
                        ...
                        <div id="div_kampus" class="hidden">
                            <label class="block text-sm font-bold text-gray-800 mb-1.5">Pilih Kendaraan Kampus <span class="text-red-500">*</span></label>
                            <select id="kendaraan_id" name="kendaraan_id" class="select2-kend w-full">
                                <option value="">-- Pilih Kendaraan --</option>
                                @foreach($kendaraans as $k)
                                    <option value="{{ $k->id }}">{{ $k->nama_kendaraan }} ({{ $k->plat_nomor }}) — Kapasitas: {{ $k->kapasitas_penumpang }} orang</option>
                                @endforeach
                            </select>
                            @if($kendaraans->isEmpty())
                                <p class="text-xs text-orange-600 mt-1.5"><i class="bi bi-exclamation-triangle mr-1"></i>Tidak ada kendaraan tersedia saat ini.</p>
                            @endif
                        </div>

                        {{-- PILIH KENDARAAN VENDOR --}}
                        <div id="div_vendor" class="hidden">
                            <label class="block text-sm font-bold text-gray-800 mb-1.5">Pilih Kendaraan Vendor <span class="text-red-500">*</span></label>
                            <select id="kendaraan_vendor_id" name="kendaraan_vendor_id" class="select2-vendor w-full">
                                ...
                                @foreach($kendaraanVendors as $v)
                                    <option value="{{ $v->id }}">{{ $v->nama_vendor }} — {{ $v->nama_kendaraan }} ({{ $v->kapasitas_penumpang }} org)</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        {{-- PENGEMUDI --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-1.5">Pengemudi <span class="text-gray-400 font-normal">(opsional)</span></label>
                            <select name="pengemudi_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition">
                        ...
                        @if($permohonan->kategori_kegiatan === 'Non SITH')
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 flex items-center gap-2">
                                ...
                                <p class="text-sm text-gray-600">Kolom anggaran tidak diperlukan karena ini adalah kegiatan Non-Dinas.</p>
                            </div>
                        @else
                            <div>
                                <label class="block text-sm font-bold text-gray-800 mb-1.5">Estimasi Biaya Operasional (Rp) <span class="text-red-500">*</span></label>
                                <div class="relative">
                        ...
                        <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                            ...
                            <a href="{{ route('spsi.alokasi') }}" class="text-sm text-gray-500 hover:text-gray-700 transition">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            $('.select2-kend, .select2-vendor').select2({ width: '100%' });
        });
        function toggleArmada() {
            const v = document.querySelector('input[name="sumber_armada"]:checked')?.value;
            const dK = document.getElementById('div_kampus');
            const dV = document.getElementById('div_vendor');
            const sK = document.getElementById('kendaraan_id');
            const sV = document.getElementById('kendaraan_vendor_id');
            if (v === 'Kampus') {
                dK.classList.remove('hidden'); dV.classList.add('hidden');
                sK.required = true; sV.required = false; sV.value = '';
            } else if (v === 'Vendor') {
                dV.classList.remove('hidden'); dK.classList.add('hidden');
                sV.required = true; sK.required = false; sK.value = '';
            } else {
                dK.classList.add('hidden'); dV.classList.add('hidden');
                sK.required = false; sV.required = false;
            }
        }
    </script>
</x-app-layout>