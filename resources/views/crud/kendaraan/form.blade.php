            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 md:p-8">
                ...
                <form action="{{ $actionRoute }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_kendaraan" value="{{ old('nama_kendaraan', $kendaraan->nama_kendaraan ?? '') }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50">
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Plat Nomor <span class="text-red-500">*</span></label>
                            <input type="text" name="plat_nomor" value="{{ old('plat_nomor', $kendaraan->plat_nomor ?? '') }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50 uppercase" placeholder="D 1234 ABC">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Kapasitas Penumpang <span class="text-red-500">*</span></label>
                            <input type="number" name="kapasitas_penumpang" value="{{ old('kapasitas_penumpang', $kendaraan->kapasitas_penumpang ?? '') }}" required min="1" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status_kendaraan" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50">
                            @foreach(['Tersedia','Maintenance','Dipinjam'] as $s)
                                <option value="{{ $s }}" {{ old('status_kendaraan', $kendaraan->status_kendaraan ?? 'Tersedia') === $s ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                    </div>
